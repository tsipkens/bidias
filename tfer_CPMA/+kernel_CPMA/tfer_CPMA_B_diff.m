...
function [Lambda,G0] = tfer_CPMA_B_diff(m_star,m,d,z,prop,varargin)
% TFER_CPMA_B_DIFF Evaluates the transfer function for a CPMA in Case B.
...
[~,G0] = kernel_CPMA.tfer_CPMA_B(m_star,m,d,z,prop,varargin{:});

rho_fun = @(G,r) (G-r)./(sqrt(2).*sig);
K_fun = @(G,r) ...
    (max(G,0)-r).*erf(rho_fun(max(G,0),r))+...
    sig.*sqrt(2/pi).*exp(-rho_fun(max(G,0),r).^2);
    % Note: max(G,0) is included to prevent numerical inaccuracies in 
    % evaluating the error function when m is sufficiently distant from m*
