...
% d = 20e-9.*ones(size(m)); % specify mobility diameter
z = 1;

prop = kernel.prop_CPMA('Olfert');
% prop.omega_hat = 1;
D_mod_vec = 1;%logspace(log10(0.01),log10(10),19);
...
sig = 2*sqrt(prop.L.*D./prop.v_bar);
Omega = D.*prop.L/(prop.del^2*prop.v_bar); % dimensionless diffusion coeff.
...
%-- Method B, alternate -------------------%
% [tfer_B_pb_alt,G0_B_pb_alt] = kernel.tfer_CPMA_B_pb_alt(prop,C3,C4,rs);
